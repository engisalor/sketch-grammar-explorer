...
for x in ttypes:
    dfAPI.loc[dfAPI["value"].eq("unknown") & dfAPI["text type"].eq(x), "value"] = (
        "Unknown/NA " + x
    )
    dfAPI.loc[dfAPI["value"].eq("===NONE===") & dfAPI["text type"].eq(x), "value"] = (
        "Unknown/NA " + x
    )

# capitalizion
for x in ["genre"]:
    dfAPI.loc[dfAPI["text type"].eq(x), "value"] = dfAPI.loc[
        dfAPI["text type"].eq(x), "value"
    ].str.capitalize()
for x in ["domain"]:
    dfAPI.loc[dfAPI["text type"].eq(x), "value"] = dfAPI.loc[
        dfAPI["text type"].eq(x), "value"
...
drops = dfAPI[
    dfAPI["text type"].isin(
        ["Author", "Title", "Keywords", "Variant", "Year", "Country"]
    )
].index
